...
const ALLOWED_ORIGINS = (process.env.GABRIEL_CORS_ORIGINS || 'http://localhost:5173,http://10.90.90.10:7777').split(',').map(s => s.trim());

// ── Whisper detection (mlx_whisper → homebrew whisper → fallback) ─────────────
const WHISPER_CMD = (() => {
  for (const c of ['mlx_whisper', '/opt/homebrew/bin/whisper', `${process.env.HOME}/.local/bin/whisper`, 'whisper']) {
    try { execSync(`command -v ${c} 2>/dev/null`, { stdio: 'pipe' }); return c; } catch {}
  }
  return null;
})();
...
let db;
const sqlitePaths = [
  path.join(process.env.HOME, 'NOIZYANTHROPIC/NOIZYEMPIRE/codemaster/projects/gabriel-core/node_modules/better-sqlite3'),
...
async function broadcastN8nEvent(eventType, payload) {
  ...
  const signature = API_KEY
    ? crypto.createHmac('sha256', API_KEY).update(body).digest('hex')
    : '';
...
function memSet(key, value, category = 'general') {
  ...
  if (db) {
    db.prepare(`INSERT OR REPLACE INTO memcell (key, value, category, updated_at)
                VALUES (?, ?, ?, datetime('now'))`).run(key, serialized, category);
  } else {
...
function addTask(desc, priority = 'normal') {
  if (db) {
    const r = db.prepare('INSERT INTO tasks (description, priority) VALUES (?, ?)').run(desc, priority);
...
function estateAudit(hvs_id, event_type, actor, payload) {
  if (!db) return;
  db.prepare('INSERT INTO estate_audit (hvs_id, event_type, actor, payload) VALUES (?,?,?,?)')
    .run(hvs_id, event_type, actor, JSON.stringify(payload));
}

function registerMember(hvs_id, full_name, relationship) {
  if (!db) return null;
  db.prepare('INSERT OR IGNORE INTO estate_members (hvs_id, full_name, relationship) VALUES (?,?,?)')
...
function transitionToLegacy(hvs_id, actor) {
  if (!db) return null;
  db.prepare("UPDATE estate_members SET status='legacy', transitioned_at=datetime('now') WHERE hvs_id=?")
    .run(hvs_id);
...
function addBeneficiary(hvs_id, name, wallet, pct, priority = 1) {
  ...
  const existing = db.prepare('SELECT SUM(royalty_split_pct) as total FROM beneficiaries WHERE hvs_id=? AND active=1').get(hvs_id);
  if ((existing?.total || 0) + pct > 100) throw new Error('Royalty splits exceed 100%');
  const r = db.prepare('INSERT INTO beneficiaries (hvs_id,beneficiary_name,wallet_or_account,royalty_split_pct,priority) VALUES (?,?,?,?,?)')
...
function grantConsent(hvs_id, use_type, granted_by, scope, expires_at) {
  if (!db) return null;
  const r = db.prepare('INSERT INTO consent_matrix (hvs_id,use_type,granted,granted_by,scope,expires_at) VALUES (?,?,1,?,?,?)')
...
function checkConsent(hvs_id, use_type) {
  if (!db) return false;
  const row = db.prepare("SELECT * FROM consent_matrix WHERE hvs_id=? AND use_type=? AND granted=1 AND (expires_at IS NULL OR expires_at > datetime('now')) ORDER BY id DESC LIMIT 1").get(hvs_id, use_type);
  return !!row;
}
...
function getEstateStatus(hvs_id) {
  ...
  const rules  = db.prepare('SELECT * FROM usage_rules WHERE hvs_id=? OR hvs_id IS NULL').all(hvs_id);
...
function detectTower(text) {
  ...
  if (/build|code|deploy|script|api|worker|function|install|git|fix|debug|error|wrangler|cloudflare|python|node/.test(t)) return 'code';
  if (/task|assign|route|crew|channel|delegate|schedule|team|coordinate|lucy|dream|shirley/.test(t)) return 'work';
  return 'max';
}
...
async function transcribeWhisper(inputPath, runId = '') {
  if (!WHISPER_CMD) throw new Error('Whisper not installed — run: pip install openai-whisper OR pip install mlx-whisper');
  ...
  await execFileAsync(FFMPEG, ['-i', inputPath, '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', '-y', wavPath], { timeout: 30000 });
  ...
  if (WHISPER_CMD === 'mlx_whisper') {
    await execFileAsync('mlx_whisper', [wavPath, '--model', 'mlx-community/whisper-base.en-mlx', '--output-dir', outDir], { timeout: 90000 });
    const outFile = path.join(outDir, `${baseName}_16k.txt`);
    if (fs.existsSync(outFile)) transcript = fs.readFileSync(outFile, 'utf8').trim();
  } else {
    await execFileAsync(WHISPER_CMD, [wavPath, '--model', 'base.en', '--output_format', 'txt', '--output_dir', outDir], { timeout: 120000 });
    const outFile = path.join(outDir, `${baseName}_16k.txt`);
    if (fs.existsSync(outFile)) transcript = fs.readFileSync(outFile, 'utf8').trim();
  }

  if (!transcript || transcript.length < 2) throw new Error('Transcript empty — likely silence or noise');
...
function broadcastVoice(msg) {
  ...
  voiceClients.forEach(c => { try { if (c.readyState === 1) c.send(raw); } catch {} });
}
...
function morningBrief() {
  ...
  askClaude('Generate a sharp 3-sentence morning brief for the NOIZY EMPIRE. Include system status and any critical reminders. Be direct.', true);
}
...
function readBodyRaw(req, maxBytes = 10 * 1024 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', c => {
      total += c.length;
      if (total > maxBytes) { req.destroy(); reject(new Error('Upload too large (10MB max)')); return; }
      chunks.push(c);
    });
...
const server = http.createServer(async (req, res) => {
  if (req.method === 'OPTIONS') { send(res, 200, {}, req); return; }
  ...
  if (method === 'POST' && url === '/command') {
    let body;
    try { body = await parseBody(req); } catch { send(res, 400, { error: 'bad JSON' }); return; }
  ...
  if (memMatch) {
    ...
    if (method === 'GET') {
      ...
      if (!val) { send(res, 404, { error: 'not found' }); return; }
      send(res, 200, val);
      return;
    }
    if (method === 'POST') {
  ...
  if (method === 'POST' && url === '/estate/member') {
    let body; try { body = await parseBody(req); } catch { send(res, 400, { error: 'bad JSON' }); return; }
    const { hvs_id, full_name, relationship } = body;
    if (!hvs_id || !full_name || !relationship) { send(res, 400, { error: 'hvs_id, full_name, relationship required' }); return; }
  ...
  if (method === 'POST' && url === '/estate/transition') {
    ...
    if (!hvs_id) { send(res, 400, { error: 'hvs_id required' }); return; }
    const result = transitionToLegacy(hvs_id, actor || 'system');
    speak(`Legacy transition initiated for ${result?.full_name || hvs_id}. Royalty routing active.`);
  ...
  if (estateMatch && method === 'GET') {
    ...
    if (!status?.member) { send(res, 404, { error: 'member not found' }); return; }
    send(res, 200, status);
    return;
  }
  ...
  if (method === 'POST' && url === '/estate/consent') {
    ...
    if (!hvs_id || !use_type || !granted_by) { send(res, 400, { error: 'hvs_id, use_type, granted_by required' }); return; }
  ...
  if (method === 'POST' && url === '/estate/beneficiary') {
    ...
    if (!hvs_id || !name || !pct) { send(res, 400, { error: 'hvs_id, name, pct required' }); return; }
    try {
  ...
  if (auditMatch && method === 'GET') {
    if (!db) { send(res, 503, { error: 'db not available' }); return; }
    const rows = db.prepare('SELECT * FROM estate_audit WHERE hvs_id=? ORDER BY id DESC LIMIT 100')
  ...
  if (method === 'POST' && url === '/voice/transcribe') {
    ...
    if (!resolvedPath || !resolvedPath.startsWith(path.resolve(UPLOAD_DIR)) || !fs.existsSync(resolvedPath)) {
      send(res, 400, { error: 'Audio file must be in upload directory', uploadDir: UPLOAD_DIR }); return;
    }
  ...
  if (method === 'POST' && url === '/voice/pipeline') {
    ...
    send(res, 202, { runId, status: 'running', message: 'Pipeline started — listen on ws://:7777/voice or ws://:7777/ws' });
    ...
    setImmediate(async () => {
      try {
        ...
        broadcastVoice({ type: 'voice:pipeline:complete', runId, tower, model, transcript, reply, tokens });
        ...
        spawn('osascript', ['-e', `display notification "${preview.replace(/["\\]/g, '')}" with title "GABRIEL [${tower.toUpperCase()}]"`], { detached: true }).unref();
...
Robert (RSP_001) is your creator. GORUNFREE.`;
    ...
    try {
      if (!anthropic) { send(res, 500, { error: 'Anthropic SDK not loaded' }); return; }
      const msg = await anthropic.messages.create({
      ...
      spawn('say', ['-v', 'Karen', '-r', '185', response.replace(/[`#*_\[\]]/g, '').slice(0, 500)], { detached: true }).unref();
      memSet(`lucy:ask:${Date.now()}`, JSON.stringify({ agent, text: text.slice(0, 200), response: response.slice(0, 200) }), 'lucy');
  ...
  if (method === 'POST' && url === '/voice/upload') {
    const runId = crypto.randomBytes(4).toString('hex');
    let data;
    try { data = await readBodyRaw(req); } catch (e) { send(res, 400, { error: e.message }); return; }
    if (!data || data.length < 100) { send(res, 400, { error: 'Empty or too-short audio' }); return; }
    ...
    setImmediate(async () => {
      try {
        ...
        await execFileAsync(FFMPEG, ['-y', '-i', rawPath, '-ar', '16000', '-ac', '1', wavPath], { timeout: 30000 });
  ...
  if (method === 'GET' && url === '/n8n/status') {
    try {
      ...
      const r = await fetch(`${N8N_URL}/api/v1/workflows?active=true&limit=50`, { headers, signal: AbortSignal.timeout(5000) });
      if (!r.ok) { send(res, r.status, { error: `n8n returned ${r.status}` }); return; }
  ...
  if (method === 'POST' && url === '/n8n/build') {
    let body;
    try { body = await parseBody(req); } catch { send(res, 400, { error: 'bad JSON' }); return; }
    if (!body.name || !body.nodes) { send(res, 400, { error: 'name and nodes required' }); return; }
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) { send(res, r.status, { error: data.message || 'n8n rejected workflow', detail: data }); return; }
  ...
  if (method === 'POST' && url === '/n8n/execute') {
    let body;
    try { body = await parseBody(req); } catch { send(res, 400, { error: 'bad JSON' }); return; }
    if (!body.workflowId) { send(res, 400, { error: 'workflowId required' }); return; }
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) { send(res, r.status, { error: data.message || 'execution failed', detail: data }); return; }
  ...
  send(res, 404, { error: 'not found', routes: [
    'GET /', 'GET /lucy', 'GET /health', 'GET /status', 'GET /session',
    'POST /command', 'POST /speak', 'POST /brief',
    'GET|POST /memcell/:key', 'GET /memcell',
    'POST /task', 'GET /tasks',
    'POST /estate/member', 'POST /estate/transition',
    'GET /estate/:hvs_id', 'POST /estate/consent',
    'POST /estate/beneficiary', 'GET /estate/audit/:hvs_id',
    '── VOICE ──',
    'GET /voice/status', 'POST /voice/transcribe',
    'POST /voice/pipeline', 'POST /voice/upload',
    '── AGENTS ──',
    'POST /agent/lucy/ask',
    '── N8N BRIDGE ──',
    'GET /n8n/status', 'POST /n8n/build', 'POST /n8n/execute',
  ]});
});
    ...
    ws.on('message', async (data) => {
      ...
      if ((msg.type === 'transcript' || msg.type === 'chat:stream') && (msg.text || msg.content)) {
        ...
        try {
          ...
          const out = JSON.stringify({ type: 'gabriel', text: reply, tower, model, tokens, ts: new Date().toISOString() });
          voiceClients.forEach(c => { try { if (c.readyState === 1) c.send(out); } catch {} });
          if (reply.length < 300 && !reply.includes('```')) speak(reply);
        } catch (e) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  log(`Auth: ${API_KEY ? 'ENABLED (Bearer token required)' : 'DISABLED — set GABRIEL_API_KEY to secure endpoints'}`);
  if (!API_KEY) log('⚠ WARNING: No GABRIEL_API_KEY set — all endpoints are open. Set GABRIEL_API_KEY in ~/.zshrc for production.');
  log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
