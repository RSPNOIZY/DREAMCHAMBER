// ============================================================
// NOISY BOX — Immutable Audit Ledger
...
export class ImmutableAuditLedger {
  ...
  async verifyChain(): Promise<{ valid: boolean; entries: number; broken_at?: string }> {
    ...
    for (const entry of entries.results) {
      if (entry.previous_hash !== expectedPrevious) {
        return { valid: false, entries: entries.results.length, broken_at: entry.id };
      }
  ...
  async getStats(): Promise<{ total_entries: number; first_entry: string | null; last_entry: string | null; chain_valid: boolean }> {
    ...
    return {
      total_entries: count?.count || 0,
      first_entry: first?.timestamp || null,
      last_entry: last?.timestamp || null,
      chain_valid: chain.valid,
    };
  }

  async getByResource(resourceType: string, resourceId: string): Promise<AuditLogEntry[]> {
    const result = await this.db
      .prepare('SELECT * FROM audit_log WHERE resource_type = ? AND resource_id = ? ORDER BY timestamp ASC')
