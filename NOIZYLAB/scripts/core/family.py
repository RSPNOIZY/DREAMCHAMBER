...
SEED = {
    ...
    "members": {
        ...
        "micky_p": {
            ...
            "owner": "RSP_001",
            "machine_class": "MacBook Pro (Apple Silicon)",
            "lan": {
                "ip": "10.0.0.100",
            ...
            "scope_constraints": {
                "macos_version": "Catalina 10.15 (Logic Pro for Mac requires 14.4+, so MICKY-P CANNOT run current Logic — that role lives on GOD)",
                "audio_hijack": "Audio Hijack 4 supports macOS 10.15+ — confirmed compatible, this is MICKY-P's primary role",
                "cloudflared": "DO NOT run Tunnel/Zero Trust here — cloudflared past 2025.6.0 segfaults on macOS Catalina 10.15.7. Tunnel stays on GOD only.",
                "security_role": "Avoid public-facing or security-critical roles. Even with Apple's 2026-02-02 Security Update 2026-001 Catalina, this is not the right machine for the trust perimeter.",
            },
            "approved_uses": [
                "Apollo Quad / U87 → Audio Hijack capture/routing → LAN bridge → GOD (PRIMARY)",
                "Legacy ingest + archive prep (clean old sessions, copy/archive, prep Aquarium uploads)",
                "Notes/logging/operator companion (Docs, Notion, terminal logs, runbook review)",
                "Light local utilities (shell scripts, SSH, browser dashboards, lightweight automation)",
            ],
            "forbidden_uses": [
                "Current Logic Pro for Mac (requires macOS 14.4+, Catalina too old)",
                "Cloudflare Tunnel / cloudflared connector (segfault bug on Catalina 10.15.7)",
                "Public-facing services or security perimeter roles",
                "Heaven worker hosting, Receipt Spine primary node",
            ],
            "honoree": False,
        },
        "apollo_quad": {
