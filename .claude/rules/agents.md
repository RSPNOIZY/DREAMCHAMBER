...
All MCP servers use stdio transport for local operation on GOD.local.
Configure in Claude Code / Windsurf MCP settings:
...
```json
{
  ...
  "dreamchamber-audio-mcp": { "command": "python", "args": ["-m", "dreamchamber_audio_mcp.server"] }
}
```
