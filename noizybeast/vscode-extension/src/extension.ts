...
import { NoizyTurboProvider } from './turboProvider';
import { NoizyOrdersProvider } from './ordersProvider';
import { NoizyEmpirePanel } from './empirePanel';
import { NoizyStatusBar } from './statusBar';

// ── CONFIG ─────────────────────────────────────────────────
function cfg<T>(key: string): T {
  return vscode.workspace.getConfiguration('noizybeast').get<T>(key)!;
}

let statusBar: NoizyStatusBar;
...
function connectGabrielEdge(ctx: vscode.ExtensionContext) {
  if (gabrielWs && gabrielWs.readyState === WebSocket.OPEN) return;
  try {
...
export function sendToGabriel(type: string, payload?: unknown) {
...
export function activate(ctx: vscode.ExtensionContext) {
  ...
  // Status bar
  statusBar = new NoizyStatusBar(ctx, getCfg());
  statusBar.start();

  // Tree views
  const turboProvider = new NoizyTurboProvider();
  const ordersProvider = new NoizyOrdersProvider();
  ...
  reg(ctx, 'noizybeast.openPanel',   () => NoizyEmpirePanel.show(ctx));
  ...
  vscode.window.showInformationMessage('⚡ NOIZYBEAST activated — GABRIEL V4 online', 'Open Empire Panel', 'T2 Flow Sync')
    .then(choice => {
      if (choice === 'Open Empire Panel') vscode.commands.executeCommand('noizybeast.openPanel');
      if (choice === 'T2 Flow Sync') cmdTurbo('T2');
    });
}
...
async function cmdStatus() {
  const panel = vscode.window.createWebviewPanel(
    'noizyStatus', 'NOIZYBEAST — Empire Status',
...
async function gabrielPing() {
  ...
  const url = cfg<string>('gabrielUrl') + '/health';
  try {
...
// ── TURBO RUNNER ───────────────────────────────────────────
async function cmdTurbo(turbo: string, showOutput = true, arg?: string) {
  const noizyLab = cfg<string>('noizyLabPath').replace('~', process.env.HOME || '');
  ...
  try {
    const res = await fetch(`${cfg<string>('gabrielUrl')}/turbo`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ turbo, arg, operator: cfg<string>('operator'), source: 'vscode-extension' }),
      signal: AbortSignal.timeout(15000),
    });
    if (res.ok) {
      const data = await res.json();
      const out = (data as any).output || (data as any).result || JSON.stringify(data, null, 2);
      outputChannel.appendLine(out);
      if (showOutput) vscode.window.showInformationMessage(`✅ ${turbo} complete`);
  ...
  proc.on('close', code => {
    ...
    if (showOutput && code === 0) vscode.window.showInformationMessage(`✅ ${turbo} complete`);
    if (code !== 0) vscode.window.showErrorMessage(`❌ ${turbo} failed — see NOIZYBEAST output`);
    statusBar.flash(turbo);
  });
}

// ── SYSTEM TURBO SCRIPT RUNNER ─────────────────────────────
function runTurboScript(scriptName: string, label: string) {
  const noizyLab = cfg<string>('noizyLabPath').replace('~', process.env.HOME || '');
  ...
  proc.on('close', (code: number) => {
    ...
    if (code === 0) vscode.window.showInformationMessage(`${status} ${label}`);
    else vscode.window.showErrorMessage(`❌ ${scriptName} failed — see NOIZYBEAST output`);
  });
}
...
async function cmdActivateVoice() {
  const gabrielUrl = cfg<string>('gabrielUrl');
...
async function cmdSendToGabriel() {
  ...
  try {
    const res = await fetch(`${cfg<string>('gabrielUrl')}/command`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function cmdDeployWorker() {
  ...
  proc.on('close', code => {
    ...
    if (code === 0) vscode.window.showInformationMessage(`${pick} deployed to edge`);
    else vscode.window.showErrorMessage(`${pick} deploy failed`);
...
async function cmdLucyAsk() {
  ...
  try {
    const res = await fetch(`${cfg<string>('gabrielUrl')}/agent/lucy/ask`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function cmdCommandCenter(ctx: vscode.ExtensionContext) {
  const commands = [
...
// ── HELPERS ────────────────────────────────────────────────
function reg(ctx: vscode.ExtensionContext, cmd: string, fn: () => void) {
...
function getStatusHtml(cfg: ReturnType<typeof getCfg>): string {
